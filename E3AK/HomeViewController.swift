...
class HomeViewController: BLE_ViewController{
    ...
    @IBOutlet weak var infoOperating: UIImageView!
    @IBOutlet weak var auto_range_setting: UILabel!
...
//            StopScanningTimer()
    ...
    public override func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if((RSSI.intValue <= 0) && (RSSI.intValue >= Config.BLE_RSSI_MIN)) {
            ...
            var tmp: DeviceInfo = DeviceInfo(UUID: uuid, name: name, peripheral: peripheral, rssi: RSSI.intValue, current_level: Convert_RSSI_to_LEVEL(RSSI.intValue), expect_level: expect_level, alive: 18)
            ...
            if( deviceInfoList.contains(tmp)) {
                ...
                auto_range_setting.text = self.GetSimpleLocalizedString("Proximity Read Range Settings") + " : " + "\(deviceInfoList[0].expect_level)"
                //Save to DB
        ...
        if !isForce{
            ...
            if forceDevice.peripheral.identifier.uuidString == uuid.uuidString {
                if( deviceInfoList.contains(forceDevice)) {
                    ...
                    forceDevice.expect_level = expect_level
                    //    print("RSSI: \(RSSI.intValue), LEVEL: \(tmp.current_level)")
                    ...
                    auto_range_setting.text = self.GetSimpleLocalizedString("Proximity Read Range Settings") + " : " + "\(deviceInfoList[du_idx].expect_level)"
...
//                }
    ...
    @objc func didEnterBG(){
        StopScanningTimer()
        for var d in deviceInfoList
        {
            let du_idx:Int = deviceInfoList.index(of: d)!
            d.current_level = 999
            deviceInfoList[du_idx] = d;
        }
...
//            StopScanningTimer()
    ...
    func changeViewContentSettings() {
        ...
        case .DeviceFound:
            ...
            deviceTypeLabel.isHidden = false
            ...
            doorStatusLabel.text = self.GetSimpleLocalizedString("DOOR CLOSED")
            doorStatusLabel.textColor = HexColor("000000")
            loadingImageView.isHidden = true
            loadingImageView.stopRotate()
            if isForce {
...
//                Config.bleManager.ScanBLEStop()
    ...
    func start_anima(){
//        UIView.animate(withDuration: 0.2, delay: 0, options: UIViewAnimationOptions.repeat, animations: {
//            self.infoOperating.transform = self.infoOperating.transform.rotated(by: CGFloat(M_PI))
//            }, completion: nil)
        self.infoOperating.isHidden = false
        ...
        if let aAnimation = rotationAnimation {
            self.infoOperating.layer.add(aAnimation, forKey: "rotationAnimation")
        }
    ...
    func stop_anime(){
        self.infoOperating.layer.removeAllAnimations()
        self.infoOperating.isHidden = true
    }
